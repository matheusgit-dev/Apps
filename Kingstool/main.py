...
class MainApp(ctk.CTk):
    ...
    def set_accent(self, new_color):
        ...
        for active_id, bar in self.sidebar_indicators.items():
            if active_id == self.active_tab:
    ...
    def create_body(self):
        ...
        self.sidebar_divider = ctk.CTkFrame(self.body_frame, width=2, corner_radius=0, fg_color="#242424")
        self.sidebar_divider.pack(side="left", fill="y")

        self.add_sidebar_button("home", "Início", pady=(15, 4))
    ...
    def add_sidebar_button(self, app_id, title, pady=4):
        container = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        container.pack(fill="x", pady=pady)

        indicator = ctk.CTkFrame(container, width=4, height=38, corner_radius=2, fg_color="transparent")
        indicator.pack(side="left", padx=(5, 5))
        self.sidebar_indicators[app_id] = indicator
        ...
        btn.pack(side="right", fill="x", expand=True, padx=(0, 10))
        self.sidebar_buttons[app_id] = btn
    ...
    def load_modules(self):
        for mod in self.modules_list:
            ...
            self.add_sidebar_button(app_id, info["title"], pady=4)
    ...
    def update_active_button(self, active_id):
        self.active_tab = active_id
        for btn_id, btn in self.sidebar_buttons.items():
            indicator = self.sidebar_indicators.get(btn_id)
            if btn_id == active_id:
                btn.configure(
                    fg_color="#0C0C0C", 
                    text_color="#FEFEFE", 
                ...
                if indicator:
                    indicator.configure(fg_color=self.accent_color)
            else:
                btn.configure(
                    fg_color="transparent", 
                    text_color="#C0C0C0", 
